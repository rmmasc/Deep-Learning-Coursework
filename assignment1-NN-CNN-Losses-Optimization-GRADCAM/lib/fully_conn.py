...
class TestFCReLU(Module):
    def __init__(self, keep_prob=0, dtype=np.float32, seed=None):
        self.net = sequential(
            ...
            fc(np.prod([ 4, 5]), output_dim=10, init_scale=0.02, name="fc_1"),
            relu(name="relu_1")
            ########### END ###########
        )
...
class SmallFullyConnectedNetwork(Module):
    def __init__(self, keep_prob=0, dtype=np.float32, seed=None):
        self.net = sequential(
            ########## TODO: ##########
            flatten(name="flatten_1"),
            fc(4, output_dim=30, init_scale=0.02, name="fc_1"),
            relu(name="relu_1"),
            fc(30, output_dim=7, init_scale=0.02, name="fc_2"),
            relu(name="relu_2")
            ########### END ###########
        )
...
class TinyNet(Module):
    def __init__(self, keep_prob=0, dtype=np.float32, seed=None):
        ...
        self.net = sequential(
            ########## TODO: ##########
            flatten(name="flat"),
            fc(32*32*3, 100, 1e-2, name="fc_1"),
            dropout(keep_prob, seed=seed),
            relu(name="relu1"),
            fc(100, 10, 1e-2, name="fc_2")
            ########### END ###########
        )
