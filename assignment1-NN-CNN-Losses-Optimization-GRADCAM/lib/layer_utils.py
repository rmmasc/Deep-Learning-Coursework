...
class flatten(object):
    ...
    def backward(self, dprev):
        ...
        #############################################################################
        #print(feat.shape)
        dfeat = dprev.reshape(tuple(feat.shape))
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = None
...
class fc(object):
    ...
    def forward(self, feat):
        ...
        #############################################################################
        output = np.zeros((feat.shape[0],self.output_dim))
        for i in range(feat.shape[0]):
             temp = np.expand_dims(feat[i],0)
             output[i] = np.matmul(temp,self.params[self.w_name]) + self.params[self.b_name]
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = feat
    ...
    def backward(self, dprev):
        ...
        # corresponding name.                                                       #
        # Store the output grad1ients in the variable dfeat provided above.          #
        #############################################################################
...
class relu(object):
    ...
    def forward(self, feat):
        ...
        # Store the results in the variable output provided above.                  #
        #############################################################################
        output = np.multiply(feat>0,feat)
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = feat
    ...
    def backward(self, dprev):
        ...
        #############################################################################
        dfeat = np.multiply(feat>0,dprev)
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = None
...
class dropout(object):
    ...
    def forward(self, feat, is_training=True, seed=None):
        ...
        if not is_training: #include all activations in case of testing phase
            mask = np.ones_like(feat)
        if is_training:
            if self.keep_prob == 0: 
                mask = np.ones_like(feat)
            else:
                mask = (self.rng.uniform(0,1,feat.shape) < self.keep_prob)/self.keep_prob
    ...
    def backward(self, dprev):
        ...
        if self.keep_prob==0:
            ...
            dfeat = np.multiply(self.kept,dprev)
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        log_logit = -np.log(logit[range(0,feat.shape[0]),label])
        loss = np.sum(log_logit)
...
def softmax(feat):
    ...
    for batch in range(feat.shape[0]):
        factor = np.sum([np.exp(logit) for logit in feat[batch][...,:]])
        scores[batch,:] = np.asarray([np.exp(logit)/factor for logit in feat[batch][...,:]])
